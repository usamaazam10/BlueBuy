...
/**
 * A hero banner photo field: upload to Cloudinary (immediate, so the
 ...
 */
export function HeroBannerImageField({ value, onChange }: HeroBannerImageFieldProps) {
  ...
  async function handleSelect(file: File) {
    ...
    try {
      ...
      toast.success('Uploaded', 'Slide photo updated.');
    } catch (error) {
      const message =
  ...
  return (
    <Field
      label="Slide photo"
      required
      hint="Wide landscape image, about 1600 × 640 px. Keep important content centred — edges may crop on phones."
    >
      <div className="flex flex-col gap-3">
        <SingleImageUpload
          ...
          onRemove={() => onChange('')}
          onError={(m) => toast.error('Slide photo', m)}
          uploading={uploading}
          progress={progress}
          fit="cover"
          label="Upload slide photo"
        />
        <Input
          value={value}
          onChange={(e) => onChange(e.target.value)}
          placeholder="https://…  (or upload above)"
          aria-label="Slide photo URL"
        />
      </div>
    </Field>
  );
